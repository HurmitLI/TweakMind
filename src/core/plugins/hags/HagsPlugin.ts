...
async function verifyHagsApply(): Promise<VerificationExecutionResult> {
  const applyResult = readPendingApplyResult("hags");
  ...
  if (applyResult.status !== "success" || applyResult.applyMode !== "real") {
    return {
      ...unavailable("Only successful real HAGS Apply results can be verified in this MVP step."),
