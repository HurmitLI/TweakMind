    ...
    <div className="flex flex-1 flex-col gap-6">
      <section className="rounded-lg border border-white/70 bg-white/85 px-8 py-8 shadow-sm backdrop-blur dark:border-slate-700 dark:bg-slate-900/85">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="mb-3 text-sm font-semibold uppercase tracking-wide text-blue-700 dark:text-blue-300">About</p>
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className="rounded-full border border-blue-200 bg-blue-50 px-3 py-1 text-sm font-semibold text-blue-700 dark:border-blue-400/40 dark:bg-blue-950/40 dark:text-blue-300">
                {AppInfo.releasePhase}
              </span>
            </div>
            <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-600 dark:text-slate-300">{AppInfo.description}</p>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">{AppInfo.tagline}</p>
          </div>
      ...
      <dl className="grid gap-4 md:grid-cols-2">
        <InfoField label="Version" value={AppInfo.versionLabel} />
        <InfoField label="License" value={AppInfo.licenseName} />
        <InfoField label="Copyright" value={AppInfo.copyright} />
        <div className="rounded-lg border border-slate-200 bg-white/80 p-4 dark:border-slate-700 dark:bg-slate-900/80">
          <dt className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">GitHub repository</dt>
          <dd className="mt-2">
            <a
      ...
      <section className="rounded-lg border border-slate-200 bg-white/95 p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900/95">
        <h3 className="text-lg font-semibold tracking-tight text-slate-950 dark:text-slate-100">Alpha release</h3>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600 dark:text-slate-300">
          This Alpha build focuses on the Knowledge → Scan → Decision → Apply → Verify → Recover workflow for supported
          Windows optimizations. Features and supported targets may change before Beta.
        </p>
        <Link
